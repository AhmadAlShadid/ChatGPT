...
let agentId = "v2_agt_YwebGam_"

// 3. Paste the 'data-client-key' in the 'auth.clientKey' variable
let auth = { type: 'key', clientKey: "Z29vZ2xlLW9hdXRoMnwxMTY2NzUyMTk2MDU5MzI5MjQ2MDc6MUxZV01KanRud19zV19ET1NRclBv"};
...
const callbacks = {
    ...
    console.log("onNewMessage():", messages, type);
        ...
        // Defensive: make sure messages is valid and has at least one element
        if (!Array.isArray(messages) || messages.length === 0) {
            console.warn("No messages received or invalid format:", messages);
            return;
        }
        ...
        // Defensive: make sure msg has role/content before accessing them
        ...
        if (msg.role === "assistant" && messages.length !== 1) {
            if (type === "answer") {
                answers.innerHTML += `${timeDisplay()} - [${msg.role}] : ${msg.content}  
                    <button id='${msg.id}_plus' title='agentManager.rate() -> Rate this answer (+)'>+</button> 
                    <button id='${msg.id}_minus' title='agentManager.rate() -> Rate this answer (-)'>-</button> 
                    <br>`;
...
speechButton.addEventListener('click', () => toggleStartStop(id ="speech"))
speakButton.addEventListener('click', () => toggleStartStop())
